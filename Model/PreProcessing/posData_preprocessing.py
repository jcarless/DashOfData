...
posData = pd.DataFrame(get_daily_timeseries(), 
                           columns=[
                                 "date",
                                 "total_sales",
                                 "guests",
...
posData["guests_log_diff"] = posData.guests_log - posData.guests_log.shift(7).fillna(0)
...
posData["temp_log_diff"] = posData.temp_log - posData.temp_log.shift(7).fillna(0)

#Process severity
posData.severity.fillna(0, inplace=True)

posData["severity_diff"] = posData.severity - posData.severity.shift(7).fillna(0)

#Process humidity
posData["humidity_diff"] = posData.humidity - posData.humidity.shift(7).fillna(0)


missingDates = pd.date_range(start = start_date, end = end_date ).difference(posData.index)
